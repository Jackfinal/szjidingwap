...
Page({
  ...
  onLoad: function (options) {
    ...
    let child = app.BaseInfo.nav[2]['child'];
    let nowClassList = [];
    ...
    for (let i in child)
    {
      app.BaseInfo.nav[2]['child'][i]['hover'] = ''
      if (options.id == child[i]['id']) {
        app.BaseInfo.nav[2]['child'][i]['hover'] = 'hover'
      }
    }
    this.setData({
    ...
    let classInfo = utils.getClassInfo(options.id, nowClassList);
    console.log(nowClassList);
